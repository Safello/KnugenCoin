...
uint256 hashGenesisBlock("0xd00394e1057bdb1000fa019569aafd923f3d1f4325e3d9a154f95281435512c9");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // KnugenCoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "KnugenCoin Signed Message:\n";
...
{
    // KnugenCoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
{
    int64 nSubsidy = 3904 * COIN;

    nSubsidy >>= (nHeight / 17170);
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // KnugenCoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // KnugenCoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
{
    // KnugenCoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0xd00394e1057bdb1000fa019569aafd923f3d1f4325e3d9a154f95281435512c9");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        block.nVersion = 1;
        block.nTime    = 1396171566;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2084803512;
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("KnugenCoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("KnugenCoinMiner : ProcessBlock, block not accepted");
    }
...
void static LitecoinMiner(CWallet *pwallet)
{
    ...
    try { loop {
        ...
        printf("Running KnugenCoin with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("KnugenCoinMiner terminated\n");
        throw;
    }
}
